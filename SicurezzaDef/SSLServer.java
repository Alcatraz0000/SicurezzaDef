...
public class SSLServer
{
    ...
    static Boolean ProtocolWriteOnChain( byte []transaction,PublicKey VoterPK) throws Exception{
        ...
        if(file1.exists() && !file1.isDirectory()){
            ...
            try {
                AppendingObjectOutputStream outputStreamExist = new AppendingObjectOutputStream( new FileOutputStream("ItalyChain.txt",true));
                outputStreamExist.writeObject(VoterPK.getEncoded());
                outputStreamExist.writeObject(transaction);
                //System.out.println("PK: " + Utils.toHex(VoterPK.getEncoded()) + "transaction written to file" + " " + Utils.toHex(transaction));
            }catch (Exception e) {
                e.printStackTrace();
            }
    ...
    //VALIDATORE
    static byte[] ClientComunication(Socket sSock, PublicKey VoterPK) throws Exception{// note that the SSLSocket object is converted in a standard Socket object and henceforth we can work as for standard Java sockets
        ...
        //System.out.println(i);
        TimeUnit.MILLISECONDS.sleep(50);
        //System.out.println("\nMessaggio ricevuto: " + new String(Hex.encode(message)));
    ...
    //VALIDATORE
    static byte[] ClientComunicationRandomness(Socket sSock, PublicKey VoterPK) throws Exception{// note that the SSLSocket object is converted in a standard Socket object and henceforth we can work as for standard Java sockets
    ...
    static PublicKey obtainVoterPK(KeyStore truststore, int IDClient) throws Exception{// note that the SSLSocket object is converted in a standard Socket object and henceforth we can work as for standard Java sockets
    ...
    static void ConfirmTransaction(Socket cSock, String esito) throws Exception{// note that the SSLSocket object is converted in a standard Socket object and henceforth we can work as for standard Java sockets
    ...
    public static void main(String[] args)throws Exception{
        ...
        SSLServerSocket sSock = (SSLServerSocket)sockfact.createServerSocket(5000); // bind to port 4000
        ...
        //SSLServerSocket sSock2 = (SSLServerSocket)sockfact.createServerSocket(5001); // bind to port 4000
        ...
        TimeUnit.MILLISECONDS.sleep(5000);
        try {
            AppendingObjectOutputStream EOF = new AppendingObjectOutputStream( new FileOutputStream("ItalyChain.txt",true));
